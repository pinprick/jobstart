    ...
    def match(self):
        ...
        for i in xrange(0, l):
            ...
            if (not rmsg.in_state("completed")):
                ...
                return ret
            if ( (rmsg.ts("completed") - smsg.ts("pending"))  < 0):
                ret = "Matching ERROR: message #" + str(i) + ": negative message transfer time. Possible reasons: (a) corrupted logs (check error output); or (b) corrupted Global Time (mpisync) file"
                return ret
